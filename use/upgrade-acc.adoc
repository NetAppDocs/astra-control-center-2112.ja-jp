...
NOTE: Astra バージョンの変更は、 Astra Control Center のアップグレードの唯一の要件です。レジストリパスは、のイメージをプッシュしたレジストリパスと一致する必要があります ,前の手順。
...
NOTE: を用意しておく必要があります  up acc-helm-repo to upgrade Traefik and Cert-manager,acc-helm -repo をセットアップします 次の手順を実行する前に、
...
NOTE: の設定が完了している必要があります  Traefik service using acc-helm-repo,Traefik アップデート および  up acc-helm-repo to upgrade Traefik and Cert-manager,Helm に acc-helm-repo が追加されました 次の手順を実行する前に、
